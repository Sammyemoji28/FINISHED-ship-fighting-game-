...
def redMove(keysPressed, red):
    if keysPressed[pygame.K_UP] and red.y - VEL > 0:
        red.y -= VEL
    if keysPressed[pygame.K_DOWN] and red.y + VEL + red.height < HEIGHT - 20:
        red.y += VEL
...
def displayWinner(winnerText):
    winnerText = WINNER_FONT.render(winnerText, 1, WHITE)
    ...
    while run:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_LCTRL and len(yellowB) < MAX_BULLET:
                    bullet = pygame.Rect(yellow.x + yellow.width, yellow.y + yellow.height//2, 10, 5)
                    yellowB.append(bullet)
                    BULLET_FIRE.play()
                if event.key == pygame.K_RCTRL and len(redB) < MAX_BULLET:
        ...
        handleBullets(redB, yellowB, red, yellow)
